...
class basic_sparse_set {
  ...
  void swap_at(const std::size_t from, const std::size_t to) {
	...
	sparse_ref(lhs) =
		traits_type::combine(static_cast<typename traits_type::entity_type>(to), traits_type::to_integral(lhs));
	sparse_ref(rhs) =
		traits_type::combine(static_cast<typename traits_type::entity_type>(from), traits_type::to_integral(rhs));
 ...
 private:
  ...
  virtual void swap_or_move([[maybe_unused]] const std::size_t lhs, [[maybe_unused]] const std::size_t rhs) {
	ENTT_ASSERT((mode != deletion_policy::swap_only) || (((lhs < free_list()) + (rhs < free_list())) != 1u),
  ...
  virtual basic_iterator try_emplace(const Entity entt, const bool force_back, const void * = nullptr) {
	...
	switch (mode) {
	  case deletion_policy::in_place:
		...
		[[fallthrough]];
	  case deletion_policy::swap_and_pop:packed.push_back(entt);
		ENTT_ASSERT(elem == null, "Slot not available");
 ...
 public:
  ...
  [[nodiscard]] iterator begin(int) const noexcept {
	return (mode == deletion_policy::swap_only) ? (end() - static_cast<typename iterator::difference_type>(head))
												: begin();
  }
  ...
  void compact() {
	if (mode == deletion_policy::in_place) {
	  ...
	  while (pos != traits_type::to_entity(null)) {
		if (const auto to = static_cast<size_type>(std::exchange(pos, traits_type::to_entity(packed[pos]))); to
			< from) {
		  --from;
  ...
  void sort_n(const size_type length, Compare compare, Sort algo = Sort{}, Args &&...args) {
	ENTT_ASSERT((mode != deletion_policy::in_place) || (head == traits_type::to_entity(null)),
				"Sorting with tombstones not allowed");
