...
class as_view {
  ...
  auto dispatch(get_t<Get...>, exclude_t<Exclude...>) const {
	return reg.template view<constness_as_t<typename Get::value_type,
											Get>...>(exclude_t<constness_as_t<typename Exclude::value_type,
																			  Exclude>...>{});
  }
...
class as_group {
  ...
  auto dispatch(owned_t<Owned...>, get_t<Get...>, exclude_t<Exclude...>) const {
	if constexpr (std::is_const_v<registry_type>) {
	  ...
	  return reg.template group<constness_as_t<typename Owned::value_type,
											   Owned>...>(get_t<constness_as_t<typename Get::value_type, Get>...>{},
														  exclude_t<constness_as_t<typename Exclude::value_type,
																				   Exclude>...>{});
	}
  }
...
auto to_entity(const basic_storage<Args...> &storage,
			   const typename basic_storage<Args...>::value_type &instance) -> typename basic_storage<Args...>::entity_type {
  ...
  for (auto it = base.rbegin(), last = base.rend(); it < last; it += page_size) {
	if (const auto dist = (addr - std::addressof(storage.get(*it))); dist >= 0
		&& dist < static_cast<decltype(dist)>(page_size)) {
	  return *(it + dist);
	}
  }
...
[[deprecated("use storage based to_entity instead")]] typename basic_registry<Args...>::entity_type to_entity(const basic_registry<
	Args...> &reg, const Component &instance) {
...
struct sigh_helper<Registry, Type> final : sigh_helper<Registry> {
  ...
  auto on_construct(Args &&...args) {
	this->registry().template on_construct<Type>(name).
		template connect<Candidate>(std::forward<Args>(args)...);
	return *this;
  }
  ...
  auto on_update(Args &&...args) {
	this->registry().template on_update<Type>(name).
		template connect<Candidate>(std::forward<Args>(args)...);
	return *this;
  }
  ...
  auto on_destroy(Args &&...args) {
	this->registry().template on_destroy<Type>(name).
		template connect<Candidate>(std::forward<Args>(args)...);
	return *this;
  }
