...
[[nodiscard]] bool all_of_but(const std::size_t index,
							  const Type *const *it,
							  const std::size_t len,
							  const Entity entt) noexcept {
  std::size_t pos{};
...
[[nodiscard]] Result view_pack(const View &view,
							   const Other &other,
							   std::index_sequence<VGet...>,
							   std::index_sequence<VExclude...>,
							   std::index_sequence<OGet...>,
							   std::index_sequence<OExclude...>) {
  Result elem{};
  // friend-initialization, avoid multiple calls to refresh
  elem.pools = {view.template storage<VGet>()..., other.template storage<OGet>()...};
  elem.filter =
	  {view.template storage<sizeof...(VGet) + VExclude>()..., other.template storage<sizeof...(OGet) + OExclude>()...};
  elem.refresh();
  return elem;
}
...
class view_iterator final {
  ...
  [[nodiscard]] bool valid(const typename iterator_type::value_type entt) const noexcept {
	return ((Get != 1u) || (entt != tombstone)) && all_of_but(index, pools.data(), Get, entt)
 ...
 public:
  ...
  view_iterator(iterator_type curr,
				...
				std::array<const Type *, Exclude> excl,
...
struct extended_view_iterator final {
  ...
  [[nodiscard]] reference operator*() const noexcept {
	return std::apply([entt = *it](auto *...curr) {
	  return std::tuple_cat(std::make_tuple(entt),
							curr->get_as_tuple(entt)...);
	}, pools);
  }
...
class basic_common_view {
  ...
  friend Return internal::view_pack(const View &,
									const Other &,
									std::index_sequence<VGet...>,
									std::index_sequence<VExclude...>,
									std::index_sequence<OGet...>,
 ...
 public:
  ...
  [[nodiscard]] bool contains(const entity_type entt) const noexcept {
	if (leading) {
	  ...
	  return (!(idx < 0 || idx > leading->begin(0).index())) && internal::all_of_but(index, pools.data(), Get, entt)
		  && internal::none_of(filter.data(), Exclude, entt);
	}
...
class basic_view<get_t<Get...>, exclude_t<Exclude...>>
	: public basic_common_view<std::common_type_t<typename Get::base_type..., typename Exclude::base_type...>,
							   sizeof...(Get),
							   sizeof...(Exclude)> {
  using base_type = basic_common_view<std::common_type_t<typename Get::base_type..., typename Exclude::base_type...>,
									  sizeof...(Get),
									  sizeof...(Exclude)>;
  ...
  void each(Func &func, std::index_sequence<Index...>) const {
	for (const auto curr : storage<Curr>()->each()) {
	  if (const auto entt = std::get<0>(curr); ((sizeof...(Get) != 1u) || (entt != tombstone))
		  && internal::all_of_but(this->index, this->pools.data(), sizeof...(Get), entt)
 ...
 public:
  ...
  void storage(Type &elem) noexcept {
	static_assert(std::is_convertible_v<Type &, type_list_element_t<Index, type_list<Get..., Exclude...>> &>,
				  "Unexpected type");
  ...
  [[nodiscard]] decltype(auto) get(const entity_type entt) const {
	if constexpr (sizeof...(Index) == 0) {
	  return std::apply([entt](auto *...curr) { return std::tuple_cat(curr->get_as_tuple(entt)...); },
  ...
  [[nodiscard]] iterable each() const noexcept {
	...
	return {internal::extended_view_iterator{base_type::begin(), as_pools},
			internal::extended_view_iterator{base_type::end(), as_pools}};
  }
  ...
  [[nodiscard]] auto operator|(const basic_view<get_t<OGet...>, exclude_t<OExclude...>> &other) const noexcept {
	return internal::view_pack<basic_view<get_t<Get..., OGet...>, exclude_t<Exclude..., OExclude...>>>(
		*this,
		...
		std::index_sequence_for<Exclude...>{},
		std::index_sequence_for<OGet...>{},
