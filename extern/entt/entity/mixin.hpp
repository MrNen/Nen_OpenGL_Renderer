...
class basic_sigh_mixin final : public Type {
  ...
  using basic_registry_type = basic_registry<typename underlying_type::entity_type,
											 typename underlying_type::base_type::allocator_type>;
  using sigh_type = sigh<void(owner_type &, const typename underlying_type::entity_type),
  ...
  void pop_all() final {
	if (auto &reg = owner_or_assert(); !destruction.empty()) {
	  for (auto it = underlying_type::base_type::begin(0), last = underlying_type::base_type::end(0); it != last;
		   ++it) {
  ...
  underlying_iterator try_emplace(const typename underlying_type::entity_type entt,
