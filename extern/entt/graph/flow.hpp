...
class basic_flow {
  ...
  using task_container_type = dense_set<id_type,
										...
										typename alloc_traits::template rebind_alloc<id_type>>;
  ...
  using adjacency_matrix_type = adjacency_matrix<directed_tag,
												 typename alloc_traits::template rebind_alloc<std::size_t>>;
	  ...
	  while (it != last) {
		if (it->second) {
		  ...
		  if (auto curr = it++; it != last) {
			...
			} else if (const auto next = std::find_if(it, last, [](const auto &value) { return value.second; }); next
				!= last) {
			  for (; it != next; ++it) {
				matrix.insert(curr->first, it->first);
