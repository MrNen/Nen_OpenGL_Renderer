...
[[nodiscard]] auto meta_arg_node(const meta_context &context,
								 type_list<Args...>,
...
[[nodiscard]] inline auto try_convert(const meta_context &context,
									  const meta_type_node &from,
									  const type_info &to,
  ...
  if (from.details) {
	...
	for (auto &&curr : from.details->base) {
	  if (auto other = try_convert(context,
								   curr.second.type(context),
								   to,
								   arithmetic_or_enum,
								   curr.second.cast(instance),
								   func); other) {
		return other;
	  }
	}
...
[[nodiscard]] meta_type_node resolve(const meta_context &context) noexcept {
  ...
  if constexpr (std::is_arithmetic_v<Type>) {
	node.conversion_helper = +[](void *bin, const void *value) {
	  return bin ? static_cast<double>(*static_cast<Type *>(bin) =
										   static_cast<Type>(*static_cast<const double *>(value)))
				 : static_cast<double>(*static_cast<const Type *>(value));
	};
  } else if constexpr (std::is_enum_v<Type>) {
	node.conversion_helper = +[](void *bin, const void *value) {
	  return bin ? static_cast<double>(*static_cast<Type *>(bin) =
										   static_cast<Type>(static_cast<std::underlying_type_t<Type>>(*static_cast<const double *>(value))))
				 : static_cast<double>(*static_cast<const Type *>(value));
	};
  }
  ...
  if constexpr (is_complete_v<meta_template_traits<Type>>) {
	node.templ = meta_template_node{
		...
		+[](const meta_context &area, const std::size_t index) noexcept { return meta_arg_node(area,
																							   typename meta_template_traits<
																								   Type>::args_type{},
																							   index);
		}};
  }
